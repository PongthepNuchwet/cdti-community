...
class ReportModel:
    ...
    def get_report_by_in_feed_id(self,feed_id):
        reports = [
            {"id":i.id ,"content_user":i.content_user,"content_admin":i.content_admin,"created_at":i.created_at,"feed_id":i.feed_id,"user_id":i.user_id,"status_admin":i.status_admin,"status_user":i.status_user,"content_appeal":i.content_appeal} for i in self.Report.query.filter(self.Report.feed_id.in_(feed_id)).all()
        ]
        print("get_report_by_uid",feed_id,reports)
        return reports

    def update_content_appeal_by_id(self,id,content):
        report = self.Report.query.filter_by(id=id).first()
        report.status_admin = 0
